...
#local-run#
ht = ht.head(1000000)
ht = ht.checkpoint('data/02a_f_ht_head_100kk.ht')
...
maps_table = maps_table.checkpoint('maps_table_per_variant.ht')
...
ht.write('data/02a_f_ht_final.ht')
maps_table.export('data/02a_f_maps_table.csv', delimiter=',')

ht_syn_ps = ht_syn_ps.checkpoint('data/ht_syn_ps.ht')
ht_syn_ps.show(3)

p = hl.plot.scatter(ht_syn_ps.mu_snp, ht_syn_ps.ps, xlabel='Mutation rate', ylabel='Singleton proportion')
show(p)

ggplot(ht_syn_ps, aes(x = ht_syn_ps.mu_snp, y = ht_syn_ps.ps)) + geom_point(aes(color = ht_syn_ps.context))